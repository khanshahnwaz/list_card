   ...
   useEffect(()=>handleForm)
    ...
    const formik=useFormik({
        ...
        validate:values=>{
            ...
            // console.log("photo ",values.photo.files[0])
            if (!values.photo) {
        ...
        onSubmit: async values=>{
            console.log("submitted data in AddItem form ",values)
            const data={
            ...
            props.setListItem([...props.listItem,data]);
            // props.setAddItem(false)
           console.log("list itmes",props.listItem)
    ...
    const handleImageChange = (e) => {
        ...
        if (file && file.type.startsWith('image/')) {
           formik.errors.photo=null;
        } else {
          formik.errors.photo='Please select a valid image file.';
        }
      };
    ...
    useEffect(()=>{
        ...
        if(imageError && formik.touched.photo){
    ...
    <RxCross1 className='float-right fixed -traslate-x-1/2 -translate-y-1/2 -right-1 -top-2 hover:opacity-10 cursor-pointer text-white' onClick={()=>props.setAddItem(false)} />
        ...
        <div className="bg-white h-full w-[100%] lg:w-[50%] ">
            <div className='mt-5 p-5 lg:p-10 mb-0'><p className='text-left text-4xl font-bold tracking-wide text-[#01061b]'>Add Item</p>
            <p className='mt-2 text-sm tracking-wide text-left font-semibold'>Don't want to add?<span className="text-[#01061b]">Close</span></p>
           ...
           <div className="grid gap-5">
            <div>
             ...
             <input type='file' className='border-2 border-[#bd8ce2] rounded-lg float-left mt-1 py-2 w-full' name='photo' accept='image/*' onBlur={formik.handleBlur} onChange={handleImageChange} value={formik.values.photo}></input>
             </Tippy>
             </div>
             </div>
